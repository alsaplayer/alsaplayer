...
extern int _internal_mpg123_error;
...
int do_layer3(struct frame *fr,int outmode,struct audio_info_struct *ai)
{
  ...
  for (gr=0;gr<granules;gr++) {
	...
	if (fr->stereo == 2) {
		if (ai && ai->pcm_buffer)
			memcpy(ai->pcm_buffer + written, pcm_sample, pcm_point);
		written += pcm_point;
		/* fr->write_buffer->WriteSamples(pcm_sample, pcm_point >> 2); */
	} else if (fr->stereo == 1) {
		short *new_val;
		int i;
		char tmp_buf[8192];
		short *in = (short *)pcm_sample;
		
		new_val = (short *)tmp_buf;
		for (i = 0; i < pcm_point >> 1; i++) {
				*new_val++ = *in;
				*new_val++ = *in++;
		}
		/* fr->write_buffer->WriteSamples(tmp_buf, pcm_point >> 1); */
		if (ai && ai->pcm_buffer)
			memcpy(ai->pcm_buffer + written, tmp_buf, pcm_point << 1);
		written += (pcm_point << 1);
	}	
	pcm_point = 0;
  }
