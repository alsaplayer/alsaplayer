...
int get_songlen(struct frame *fr,int no)
{
	...
	if(no < 0) {
		if(!rd || rd->filelen < 0)
			return 0;
		no = (int)((double) rd->filelen / compute_bpf(fr));
	}
...
int calc_numframes(struct frame *fr)
{
	int pos,len;
	pos=rd->tell(rd);
	len=lseek(rd->filept,0,SEEK_END);
	lseek(rd->filept,pos,SEEK_SET);
...
void stream_jump_to_frame(struct frame *fr,int frame)
{
	...
	rd->rewind(rd);
	read_frame(fr);
	lseek(rd->filept,frame*(fr->framesize+4),SEEK_CUR);
	read_frame(fr);
}
